...
class SqliteDbStore {
    ...
    let  ripaResponse = "ripaResponseTable"
   ...
   // let ripaRes = RipaResponse(question_id: questionId, response: rep, internal: inter, userid: idUser, question: questn, CreatedBy: cDate, physical_attribute: attri, key: keyS, personId: pId, description: "", question_code: qCode, cascade_ques_id: "", order_number: orderN, option_id: opId, cascade_option_id: "", main_question_id: mId)
    ...
    func insertRipaResponse(ripaRes : RipaResponse) {
        ...
        if sqlite3_prepare_v2(db, insertQuestionString, -1, &insertStatement, nil) == SQLITE_OK {
            ...
            sqlite3_bind_text(insertStatement, 7, (ripaRes.physical_attribute as NSString).utf8String , -1, nil)
            sqlite3_bind_text(insertStatement, 8, (ripaRes.key as NSString).utf8String , -1, nil)
            sqlite3_bind_text(insertStatement, 9, (ripaRes.personId as NSString).utf8String , -1, nil)
            sqlite3_bind_text(insertStatement, 10,(ripaRes.description as NSString).utf8String , -1, nil)
            sqlite3_bind_text(insertStatement, 11,(ripaRes.question_code as NSString).utf8String , -1, nil)
            sqlite3_bind_text(insertStatement, 12,(ripaRes.cascade_ques_id as NSString).utf8String , -1, nil)
            sqlite3_bind_text(insertStatement, 13,(ripaRes.order_number as NSString).utf8String , -1, nil)
            sqlite3_bind_text(insertStatement, 14,(ripaRes.option_id as NSString).utf8String , -1, nil)
    ...
    func getRipaResponse()-> RipaResponse? {
        ...
        while(sqlite3_step(stmt) == SQLITE_ROW){
            ...
            let internall = String(cString: sqlite3_column_text(stmt,2))
            let userid = String(cString: sqlite3_column_text(stmt, 3))
            let question = String(cString: sqlite3_column_text(stmt, 4))
            let CreatedBy = String(cString: sqlite3_column_text(stmt,5))
            let physical_attribute = String(cString: sqlite3_column_text(stmt, 6))
            let key = String(cString: sqlite3_column_text(stmt, 7))
            let personId = String(cString: sqlite3_column_text(stmt, 8))
            let description = String(cString: sqlite3_column_text(stmt,9))
    ...
    func insertRipaTempMaster(ripaResponse : RipaTempMaster, tableName:String) {
        ...
        var street : String = ripaResponse.street
        var intersectionStreet : String = ripaResponse.intersectionStreet
        let mainString = ripaResponse.location
        var BlockStr : String = ripaResponse.block
        if mainString.count > 2 {
            ...
            if components.count > 1 {
                ...
                let stretArr = mainString.components(separatedBy: "/")
                if stretArr.count>1 {
                    street = stretArr[0]
                    intersectionStreet = stretArr[1]
                }
        ...
        if sqlite3_prepare_v2(db, insertQuestionString, -1, &insertStatement, nil) == SQLITE_OK {
            ...
            sqlite3_bind_text(insertStatement, 11,  (street as NSString).utf8String , -1, nil)
            sqlite3_bind_text(insertStatement, 12,  (BlockStr as NSString).utf8String , -1, nil)
            sqlite3_bind_text(insertStatement, 13,  (intersectionStreet as NSString).utf8String , -1, nil)
